...
def create_todo(db: Session, todo: schemas.ToDoRequest):
    db_todo = models.ToDo(name=todo.name, completed=todo.completed) #transforms the request schema to model
    db.add(db_todo) #adds model object to db
    db.commit() #saves
    db.refresh(db_todo) #updates
    return db_todo
